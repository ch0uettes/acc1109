...
class VoteService:
    ...
    def cast_vote(self, match_id: int, voter_player_id: int, voted_player_id: int, actor_role: Role) -> Vote:
        ...
        Order: permission -> voter/candidate are actual participants of
        this match (never another match's roster) -> not already voted.
        The uniqueness check is also backed by a DB-level unique index
        (see VoteEntity/migrate_2026_vote_uniqueness.py) - a race that
        slips past the check-then-insert here still can't produce two
